classdef HardyWindModel < WindModel
    ...
    methods
        ...
        function result = getRandomWindVectors(obj, heights, ref_frame)
            ...
            for i=1:length(result)
                if obj.tropopausethickness>0
                    tmp = (heights(i)-obj.tropopauseheight)/obj.tropopausethickness;                    result{i} = Vector(groundvx+tropovx*exp(-tmp^2),...
                                       groundvy+tropovy*exp(-tmp^2),...
                                       heights(i),... % this part is different from the arroyo, might be a bug in Arroyo
                                       ref_frame);
                else
                    result{i} = Vector(groundvx,...
                                       groundvy,...
                                       0,...
                                       ref_frame);
                end
